    ...
    def _find_or_create_impl(self, path_split):
        ...
        if not match:
            ...
            for current_path in path_split:
                subdir = DirectoryTree2(current_path)
...
if __name__ == '__main__':
    root = DirectoryTree2()
    foo = DirectoryTree2('foo')
    root.add_subdir(foo)
    zz = DirectoryTree2('zz')
    foo.add_subdir(zz)
    bar = DirectoryTree2('bar')
    root.add_subdir(bar)

    xyz = DirectoryTree2('xyz')
    zz.add_subdir(xyz)

    b2 = DirectoryTree2('b2')
    bar.add_subdir(b2)
